...
func (b *Buffer) Extend(n int) []byte {
	...
	if end > b.capacity {
		panic(fmt.Sprintln("buffer overflow: capacity ", b.capacity, ",end ", b.end, ", need ", n))
	}
...
func (b *Buffer) ExtendHeader(n int) []byte {
	if b.start < n {
		panic(fmt.Sprintln("buffer overflow: capacity ", b.capacity, ",start ", b.start, ", need ", n))
	}
	b.start -= n
...
func (b *Buffer) Reserve(n int) {
	if n > b.capacity {
		panic(fmt.Sprintln("buffer overflow: capacity ", b.capacity, ", need ", n))
	}
	b.capacity -= n
}
...
func (b *Buffer) OverCap(n int) {
	if b.capacity+n > len(b.data) {
		panic(fmt.Sprintln("buffer overflow: capacity ", len(b.data), ", need ", b.capacity+n))
	}
	b.capacity += n
}
