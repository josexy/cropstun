...
func findProcessName(network string, ip netip.Addr, srcPort int) (uint32, uint32, string, error) {
	...
	pid, pp, err := resolveProcessNameByProcSearch(inode, uid)
	return uid, pid, pp, err
}
...
func resolveProcessNameByProcSearch(inode, uid uint32) (uint32, string, error) {
	...
	for _, f := range files {
		...
		for _, fd := range fds {
			...
			if bytes.Equal(buffer[:n], socket) {
				procname, err := os.Readlink(filepath.Join(processPath, "exe"))
				if err != nil {
					return 0, "", err
				}
				pid, _ := strconv.ParseUint(f.Name(), 10, 32)
				return uint32(pid), procname, nil
			}
		}
	}
